...
from app.api.schemas.build_list import BuildListRead, BuildListCreate, BuildListUpdate # Ensure this import is present
...
def create_and_login_user(client: TestClient, username_suffix: str) -> int: # Returns user_id
    ...
    if response.status_code == 200:
        ...
        raise Exception(f"Failed to create user {username} for tests. Status: {response.status_code}, Detail: {response.text}")

    # Log in to set cookie on the client
    login_data = {"username": username, "password": password}
    token_response = client.post(f"{settings.API_STR}/token", data=login_data)
    if token_response.status_code != 200:
        raise Exception(f"Failed to log in user {username} to set cookie. Status: {token_response.status_code}, Detail: {token_response.text}")
    ...
    if user_id == -1:
        me_response = client.get(f"{settings.API_STR}/users/me") # Cookie is sent by client
        if me_response.status_code == 200:
            user_id = me_response.json()["id"]
        else:
            raise Exception(f"Could not retrieve user_id for existing user {username} via /users/me. Status: {me_response.status_code}, Detail: {me_response.text}")
...
def create_car_for_user_cookie_auth(client: TestClient, car_make: str = "TestMake", car_model: str = "TestModel") -> int:
    car_data = {
        "make": car_make,
        "model": car_model,
        "year": 2023,
...
def test_create_build_list_unauthenticated(client: TestClient, db_session: Session):
    ...
    response = client.post(f"{settings.API_STR}/build_lists/", json=build_list_data) # No cookie sent
...
def test_create_build_list_for_other_users_car(client: TestClient, db_session: Session):
    ...
    _ = create_and_login_user(client, "userA_car_owner_bl") # Client has User A's cookie
    car_id_a = create_car_for_user_cookie_auth(client, "Honda", "Civic") # Uses User A's cookie

    # User B logs in (this clears User A's cookie from client and sets User B's)
    client.cookies.clear() 
    _ = create_and_login_user(client, "userB_bl_attacker") # Client now has User B's cookie
...
def test_read_build_list_success(client: TestClient, db_session: Session):
    ...
    create_response = client.post(f"{settings.API_STR}/build_lists/", json=build_list_data)
...
def test_update_own_build_list_success(client: TestClient, db_session: Session):
    ...
    create_response = client.post(f"{settings.API_STR}/build_lists/", json=build_list_data_initial) # Uses cookie
    assert create_response.status_code == 200
    build_list_id = create_response.json()["id"]

    update_payload = {"name": "Updated GT-R Build", "description": "Now with more power!"}
    response = client.put(f"{settings.API_STR}/build_lists/{build_list_id}", json=update_payload) # Uses cookie
    assert response.status_code == 200, response.text
...
def test_update_own_build_list_change_car_success(client: TestClient, db_session: Session):
    ...
    car_id_2 = create_car_for_user_cookie_auth(client, "Mitsubishi", "Evo") # User owns both cars, uses cookie
...
def test_update_build_list_unauthenticated(client: TestClient, db_session: Session):
    ...
    create_response = client.post(f"{settings.API_STR}/build_lists/", json=bl_data) # Uses cookie
    assert create_response.status_code == 200
    ...
    response = client.put(f"{settings.API_STR}/build_lists/{bl_id}", json=update_payload) # No cookie sent
    assert response.status_code == 401
...
def test_update_build_list_not_found(client: TestClient, db_session: Session):
    ...
    response = client.put(f"{settings.API_STR}/build_lists/777777", json=update_payload) # Uses cookie
    assert response.status_code == 404
    assert response.json()["detail"] == "Build List not found"
...
def test_update_other_users_build_list_forbidden(client: TestClient, db_session: Session):
    ...
    car_id_a = create_car_for_user_cookie_auth(client, "Ford", "Mustang") # Uses User A's cookie
    bl_data_a = {"name": "User A's Build", "car_id": car_id_a}
    create_response_a = client.post(f"{settings.API_STR}/build_lists/", json=bl_data_a) # Uses User A's cookie
    assert create_response_a.status_code == 200
    bl_id_a = create_response_a.json()["id"]

    # User B logs in (this clears User A's cookie from client and sets User B's)
    client.cookies.clear() 
    _ = create_and_login_user(client, "userB_bl_updater_attacker") # Client now has User B's cookie
    
    update_payload = {"name": "Malicious Update"}
    response = client.put(f"{settings.API_STR}/build_lists/{bl_id_a}", json=update_payload) # Uses User B's cookie
    assert response.status_code == 403
...
def test_update_build_list_to_other_users_car_forbidden(client: TestClient, db_session: Session):
    ...
    _ = create_and_login_user(client, "userA_bl_car_switcher") # Client has User A's cookie
    car_id_a_own = create_car_for_user_cookie_auth(client, "BMW", "M3") # Uses User A's cookie
    ...
    _ = create_and_login_user(client, "userB_car_owner_target") # Client has User B's cookie
    car_id_b_target = create_car_for_user_cookie_auth(client, "Audi", "R8") # Uses User B's cookie

    # User A logs back in to attempt the update
    client.cookies.clear() # Clear User B's cookie
    _ = create_and_login_user(client, "userA_bl_car_switcher") # Client has User A's cookie again

    update_payload = {"car_id": car_id_b_target}
    response = client.put(f"{settings.API_STR}/build_lists/{bl_id_a}", json=update_payload) # Uses User A's cookie
    assert response.status_code == 403
...
def test_update_build_list_to_non_existent_car_not_found(client: TestClient, db_session: Session):
    ...
    car_id_own = create_car_for_user_cookie_auth(client, "Porsche", "911") # Uses cookie
    ...
    response = client.put(f"{settings.API_STR}/build_lists/{bl_id}", json=update_payload) # Uses cookie
    assert response.status_code == 404
...
def test_delete_build_list_unauthenticated(client: TestClient, db_session: Session):
    ...
    response = client.delete(f"{settings.API_STR}/build_lists/{bl_id}") # No cookie sent
    assert response.status_code == 401
...
def test_delete_build_list_not_found(client: TestClient, db_session: Session):
    ...
    response = client.delete(f"{settings.API_STR}/build_lists/666666") # Uses cookie, Non-existent ID
    assert response.status_code == 404
    assert response.json()["detail"] == "Build List not found"

def test_delete_other_users_build_list_forbidden(client: TestClient, db_session: Session):
    # User A logs in and creates a car and a build list
    _ = create_and_login_user(client, "userA_bl_owner_del") # Client has User A's cookie
    car_id_a = create_car_for_user_cookie_auth(client, "Ferrari", "488") # Uses User A's cookie
    ...
    # User B logs in (this clears User A's cookie from client and sets User B's)
    client.cookies.clear()
    _ = create_and_login_user(client, "userB_bl_deleter_attacker") # Client now has User B's cookie
    response = client.delete(f"{settings.API_STR}/build_lists/{bl_id_a}") # Uses User B's cookie
    assert response.status_code == 403
