...
def test_create_build_list_success(client: TestClient, db_session: Session):
    ...
    response = client.post("/build_lists/", json=build_list_data, headers=auth_headers)
...
def test_create_build_list_unauthenticated(client: TestClient, db_session: Session):
    ...
    response = client.post("/build_lists/", json=build_list_data) # No auth_headers
...
def test_create_build_list_for_other_users_car(client: TestClient, db_session: Session):
    ...
    response = client.post("/build_lists/", json=build_list_data, headers=auth_headers_b)
...
def test_read_build_list_success(client: TestClient, db_session: Session):
    ...
    create_response = client.post("/build_lists/", json=build_list_data, headers=auth_headers)
    assert create_response.status_code == 200
    build_list_id = create_response.json()["id"]

    response = client.get(f"/build_lists/{build_list_id}") # No auth needed as per current endpoint
    assert response.status_code == 200, response.text
...
def test_read_build_list_not_found(client: TestClient, db_session: Session):
    response = client.get("/build_lists/999999") # Assuming this ID won't exist
    assert response.status_code == 404
...
def test_update_own_build_list_success(client: TestClient, db_session: Session):
    ...
    create_response = client.post("/build_lists/", json=build_list_data_initial, headers=auth_headers)
    assert create_response.status_code == 200
    build_list_id = create_response.json()["id"]

    update_payload = {"name": "Updated GT-R Build", "description": "Now with more power!"}
    response = client.put(f"/build_lists/{build_list_id}", json=update_payload, headers=auth_headers)
    assert response.status_code == 200, response.text
...
def test_update_build_list_unauthenticated(client: TestClient, db_session: Session):
    ...
    create_response = client.post("/build_lists/", json=bl_data, headers=auth_headers)
    assert create_response.status_code == 200
    bl_id = create_response.json()["id"]

    update_payload = {"name": "New Name Unauth"}
    response = client.put(f"/build_lists/{bl_id}", json=update_payload) # No auth headers
...
def test_update_build_list_not_found(client: TestClient, db_session: Session):
    ...
    response = client.put("/build_lists/777777", json=update_payload, headers=auth_headers)
    assert response.status_code == 404
    assert response.json()["detail"] == "Build List not found"
...
def test_update_other_users_build_list_forbidden(client: TestClient, db_session: Session):
    ...
    create_response_a = client.post("/build_lists/", json=bl_data_a, headers=auth_headers_a)
    ...
    response = client.put(f"/build_lists/{bl_id_a}", json=update_payload, headers=auth_headers_b)
    assert response.status_code == 403
...
def test_update_build_list_to_other_users_car_forbidden(client: TestClient, db_session: Session):
    ...
    response = client.put(f"/build_lists/{bl_id_a}", json=update_payload, headers=auth_headers_a)
    assert response.status_code == 403
...
def test_update_build_list_to_non_existent_car_not_found(client: TestClient, db_session: Session):
    ...
    response = client.put(f"/build_lists/{bl_id}", json=update_payload, headers=auth_headers)
    assert response.status_code == 404
...
def test_delete_own_build_list_success(client: TestClient, db_session: Session):
    ...
    response = client.delete(f"/build_lists/{bl_id}", headers=auth_headers)
    assert response.status_code == 200, response.text
    deleted_bl_data = response.json()
    assert deleted_bl_data["id"] == bl_id

    # Verify build list is deleted
    get_response = client.get(f"/build_lists/{bl_id}")
...
def test_delete_build_list_unauthenticated(client: TestClient, db_session: Session):
    ...
    response = client.delete(f"/build_lists/{bl_id}") # No auth headers
    assert response.status_code == 401
...
def test_delete_build_list_not_found(client: TestClient, db_session: Session):
    ...
    response = client.delete("/build_lists/666666", headers=auth_headers) # Non-existent ID
    assert response.status_code == 404
    assert response.json()["detail"] == "Build List not found"
...
def test_delete_other_users_build_list_forbidden(client: TestClient, db_session: Session):
    ...
    response = client.delete(f"/build_lists/{bl_id_a}", headers=auth_headers_b)
    assert response.status_code == 403
