...
def create_car_for_user_cookie_auth(client: TestClient, car_make: str = "TestMakePart", car_model: str = "TestModelPart") -> int:
    car_data = {"make": car_make, "model": car_model, "year": 2024, "trim": "TestTrimPart"}
    response = client.post(f"{settings.API_STR}/cars/", json=car_data) 
    assert response.status_code == 200, f"Failed to create car for part tests: {response.text}"
    return response.json()["id"]

# Helper function to create a build list for a car owned by the currently logged-in user
def create_build_list_for_car_cookie_auth(client: TestClient, car_id: int, bl_name: str = "TestBLPart") -> int:
    build_list_data = {"name": bl_name, "description": "Test BL for parts", "car_id": car_id}
    response = client.post(f"{settings.API_STR}/build_lists/", json=build_list_data)
    assert response.status_code == 200, f"Failed to create build list for part tests: {response.text}"
    return response.json()["id"]
...
def test_create_part_for_other_users_build_list_forbidden(client: TestClient, db_session: Session):
    ...
    build_list_id_a = create_build_list_for_car_cookie_auth(client, car_id_a, "UserA_S2000_BL")
...
def test_update_own_part_success(client: TestClient, db_session: Session):
    ...
    update_payload = {"name": "Carbon Fiber Spoiler", "description": "Lightweight and stylish"}
...
def test_update_own_part_change_build_list_success(client: TestClient, db_session: Session):
    ...
    car_id = create_car_for_user_cookie_auth(client)
    build_list_id_1 = create_build_list_for_car_cookie_auth(client, car_id, "BL_1_for_Part_Move")
    build_list_id_2 = create_build_list_for_car_cookie_auth(client, car_id, "BL_2_for_Part_Move") # User owns both BLs
...
def test_update_part_to_other_users_build_list_forbidden(client: TestClient, db_session: Session):
    ...
    build_list_id_a_own = create_build_list_for_car_cookie_auth(client, car_id_a, "UserA_Silvia_BL")
    ...
    build_list_id_b_target = create_build_list_for_car_cookie_auth(client, car_id_b, "UserB_AE86_BL")

    # User A logs back in to attempt the update
    client.cookies.clear()
    _ = create_and_login_user(client, "userA_part_bl_switcher")

    update_payload = {"build_list_id": build_list_id_b_target} # Attempt to move part to User B's BL
    response = client.put(f"{settings.API_STR}/parts/{part_id_a}", json=update_payload)
    assert response.status_code == 403
